...
public class CustomerChatListActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getSupportActionBar() != null) { getSupportActionBar().setDisplayHomeAsUpEnabled(true); getSupportActionBar().setTitle("Chats"); }
        toolbar.setNavigationOnClickListener(v -> finish());

        customerId = SupabaseManager.getCurrentUserId();
        if (customerId == null) { finish(); return; }
        ...
        adapter = new ChatListAdapter(chatList, chat -> {
            String otherId = chat.participants.get(0).equals(customerId) ? chat.participants.get(1) : chat.participants.get(0);
    ...
    private void loadChats() {
        SupabaseManager.getChatList(customerId, new SupabaseManager.SupabaseCallbackWithChats() {
